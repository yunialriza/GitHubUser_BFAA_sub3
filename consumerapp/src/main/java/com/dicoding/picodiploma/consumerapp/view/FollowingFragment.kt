...
import com.dicoding.picodiploma.consumerapp.adapter.FollowingAdapter
import com.dicoding.picodiploma.consumerapp.databinding.FragmentFollowingBinding
...
class FollowingFragment : Fragment() {
    ...
    private fun configModel(adapterFollowing: FollowingAdapter) {
        followingViewModel.getListFollowing().observe(viewLifecycleOwner,{
            userItemFollowing -> if (userItemFollowing != null) {
    ...
    private fun showFollowingViewModel() {
        ...
        if (checkFavoriteFrom == true) {
            ...
            followingViewModel.setDataFollowing(userFollower.username.toString(), requireActivity().applicationContext)
            showLoading(true)
        }
        else {
